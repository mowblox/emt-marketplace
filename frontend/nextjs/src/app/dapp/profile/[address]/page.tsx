...
const Profile = () => {
  ...
  const isCurrentUserProfile = user?.address === address;
  
  const { data: otherProfile } = useQuery({
    queryKey: ["profile", address],
    queryFn: () => fetchProfile(address as string),
    enabled: !isCurrentUserProfile,
  });
  ...
  const {data: isFollowingUser, } = useQuery({
    ...
    queryFn: (v) => checkFollowing(address as string),
    enabled: !isCurrentUserProfile && !!user?.address,
  })
  
  console.log('isCurrentUserProfile', isCurrentUserProfile, 'isFollowingUser', isFollowingUser, 'otherProfile', otherProfile)
  const queryClient = useQueryClient();

  // TODO: @Jovells convert this to react query v5 syntax and use react query v5

  const {mutateAsync} = useMutation( {
    mutationFn: ()=>  isFollowingUser? unfollowUser(otherProfile?.address!) :followUser(otherProfile?.address!),
    onSuccess: () => {
      queryClient.setQueryData(["isFollowing", address], (OldfollowStatus : boolean)=>{
        ...
        <div className="py-2 rounded-md flex w-full items-center justify-between">
          ...
          {isCurrentUserProfile ? (
            <Button
              variant="ghost"
              onClick={() =>
                router.push("/dapp/profile/" + profile.username + "/edit")
              }
